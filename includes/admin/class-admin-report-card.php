<?php
/**
 * Admin: Daily Reports — mobile-first Pet Report Card (build/report-card.js).
 *
 * @package KennelFlow
 */
...
/**
 * Class AdminReportCard
 */
class AdminReportCard {

	const PAGE_SLUG = 'kf-daily-reports';

	/**
	 * Script handle for compiled bundle.
	 * Distinct handle so other code cannot replace this script or localization.
	 */
	const SCRIPT_HANDLE = 'kf-hub-daily-reports';
	...
	/**
	 * Capability for sending report cards.
	 *
	 * @return string
	 */
	public static function required_cap() {
		return apply_filters( 'ltkf_admin_report_card_capability', 'edit_posts' );
	}
	...
	public static function register_menu() {
		add_submenu_page(
			...
			__( 'Daily Reports', 'kennelflow-core' ),
			__( 'Daily Reports', 'kennelflow-core' ),
			self::required_cap(),
			self::PAGE_SLUG,
	...
	/**
	 * Enqueue React bundle on Daily Reports screen only.
	 *
	...
	public static function maybe_enqueue_scripts( $hook_suffix = '' ) {
		...
		// Failsafes: `admin.php?page=kf-daily-reports` and loose hook match (avoids empty screen on some hosts).
		if ( ! $ok && is_admin() && ! wp_doing_ajax() ) {
			...
			if ( self::PAGE_SLUG === $page_get ) {
				$ok = true;
			} elseif ( '' !== (string) $hook_suffix && false !== strpos( (string) $hook_suffix, 'kf-daily-reports' ) ) {
				$ok = true;
			}
		}

		/**
		 * Whether the current admin view is the Daily Reports screen (bundle should load).
		 *
		 * @since 0.2.0
		 *
		 ...
		 */
		$ok = (bool) apply_filters( 'ltkf_is_admin_report_card_screen', $ok, (string) $hook_suffix, self::PAGE_SLUG, $expected_ids );
		if ( ! $ok ) {
			return;
		}

		$report_js = LTKF_PLUGIN_DIR . 'build/report-card.js';
		if ( ! is_readable( $report_js ) ) {
		...
		$asset_file = LTKF_PLUGIN_DIR . 'build/report-card.asset.php';
		$asset      = array(
			'dependencies' => array(),
		...
		wp_enqueue_script(
			self::SCRIPT_HANDLE,
			LTKF_PLUGIN_URL . 'build/report-card.js',
			$asset['dependencies'],
			$asset['version'],
			true
		);
		...
		wp_enqueue_style(
			self::SCRIPT_HANDLE,
			LTKF_PLUGIN_URL . 'build/report-card.css',
			array(),
			$asset['version']
		);
	}
	...
	public static function render_page() {
		...
		?>
		<div class="wrap kf-report-card-wrap">
			<h1><?php echo esc_html( get_admin_page_title() ); ?></h1>
			<p class="description"><?php esc_html_e( 'Send a photo and quick status update to the pet owner. Only pets with an active boarding stay (from the booking calendar) appear below.', 'kennelflow-core' ); ?></p>
			<div id="kf-report-card-root" class="kf-report-card-root" aria-live="polite"></div>
		</div>
		<?php
	}
}
