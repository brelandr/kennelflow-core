<?php
/**
 * Insert one valid Rabies wp_kf_medical_records row for E2E_PET_ID.
 * expiration_gmt = now UTC + 365 days; status = active when column exists.
 *
 * @package KennelFlow
 */
...
try {
	$expiration_gmt = $utc->modify( '+365 days' )->format( 'Y-m-d H:i:s' );
} catch ( Exception $e ) {
	unset( $e );
...
$meta_json = wp_json_encode( array( 'source' => 'e2e_inject_valid_rabies' ) );
if ( false === $meta_json ) {
	$meta_json = '{}';
}
